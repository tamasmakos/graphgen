...
The methodology is applied to the verbatim reports of the 'This is Europe' European Parliamentary debate series (2022-2024). In the implemented pipeline, the transcripts are first organized into a heterogeneous lexical graph of documents, line-level segments, and overlapping text chunks. Extraction labels are resolved from manually configured types and, optionally, ontology-derived class labels parsed from RDF/XML files. Each chunk then passes through a hybrid extraction stage in which a preliminary NER pass proposes entity hints and candidate types, after which an LLM performs constrained entity-relation extraction. The extracted entities and triplets are merged into a cumulative graph, and entity normalization is performed through string-based cleanup and embedding-based semantic resolution. In the iterative experimental workflow, optional **Node2Vec**-based edge weighting is then evaluated before Leiden community detection.

Recognizing that discourse topics exist at multiple granularities—a limitation noted in Section 1.1—the implemented community pipeline uses a two-level hierarchy: Leiden first identifies broad thematic clusters (macro-topics), after which a second pass is run within each parent community to derive sub-communities (sub-topics). Each resulting community is then processed through an **LLM-based summarization pipeline**: the system prompts a language model to analyze the induced subgraph—its entities, relations, and retrieved text chunks—to produce a coherent topic label and descriptive summary. This approach addresses the discourse topic hierarchy (Reinhart, 1981) more faithfully than flat clustering alone, while the LLM summarization provides interpretable labels grounded in graph structure and textual evidence rather than in post-hoc keyword lists.

A key empirical finding of this research is that post hoc analysis of saved graph artifacts suggests **scale-free tendencies** in the resulting entity graph, a hallmark of many organic complex networks. Furthermore, in the iterative experimental workflow, optional Node2Vec-based edge weighting was shown to enhance community definition, improving the Leiden modularity score from a baseline of roughly 0.74 to roughly 0.82 in later iterations. This structural enhancement helps reconcile a high average semantic similarity (0.57) among topic summaries with the identification of graph-theoretically distinct thematic clusters.
...
The transformation of unstructured text from the parliamentary debates into a structured graph is the foundational step of the methodology. In the implemented system, this is not a direct jump from raw text to an entity-only knowledge graph. Rather, the pipeline first builds a lexical scaffold and then enriches it with extracted entities and relations. The process consists of the following stages.

1.  **Label Resolution from Configuration and Optional Ontology Files:** Before extraction begins, the pipeline resolves the label inventory used during entity typing. This inventory may come from manually configured labels, from ontology-derived class labels parsed from RDF/XML files, or from a merge of the two. In this sense, the ontology functions as guidance for label selection rather than as a fully injected symbolic structure inside the graph.

2.  **Segmentation and Chunking:** The raw transcripts are first segmented line by line into discourse units. These segments are then split into overlapping character-based chunks using a recursive text splitter. The chunk size and overlap are configurable design parameters that balance extraction granularity against computational cost.

3.  **Hybrid Extraction with NER Guidance:** Each chunk is processed by a hybrid extraction workflow. A preliminary NER pass proposes candidate entity mentions and, when available, narrows the set of candidate labels for that chunk. An LLM-based extractor then performs the final constrained entity-relation extraction, producing typed nodes and relation triplets. If the NER stage yields no reliable label restriction, the extractor falls back to the broader configured label inventory.

4.  **Heterogeneous Graph Assembly:** The extracted outputs are inserted into a cumulative heterogeneous graph that retains document, segment, chunk, and entity layers. This design preserves provenance between lexical context and extracted relational structure. Community detection is later applied not to the whole lexical scaffold, but to the entity-relation subgraph derived from it.

5.  **Entity Normalization:** To reduce duplication, entity normalization occurs in two stages. First, local surface-form variants are consolidated during extraction through lightweight string-based coreference cleanup. Second, a graph-level resolution stage merges semantically similar entity nodes using embedding similarity. This normalization is essential for preventing central concepts from being fragmented across many near-duplicate nodes.

### **2.3 Post Hoc Structural Diagnostics: Scale-Free Tendencies**

One of the post hoc analytical questions raised by the extracted graph concerns its global topology. Degree-distribution analysis of saved graph artifacts suggests that the resulting entity graph exhibits scale-free tendencies, in the sense that a small number of nodes function as highly connected hubs while most nodes remain sparsely connected. In network science, such heavy-tailed degree patterns are commonly associated with organically grown complex systems (Barabási & Albert, 1999; Newman, 2005).

This observation should be interpreted carefully. In the present implementation, scale-free analysis is not a runtime validation stage embedded directly inside the core pipeline; rather, it is a later analytical diagnostic applied to saved graph outputs. Even so, it remains substantively informative. Political discourse grows cumulatively as each speech introduces new entities and attaches them to already salient themes such as Ukraine, energy security, migration, or EU unity. Such a process is plausibly consistent with the mechanisms of growth and preferential attachment described by Barabási and Albert (1999).

For that reason, the emergence of hub-dominated structure can be treated as supportive evidence that the extraction process captured meaningful discourse organization rather than merely producing a random relational artifact. It should nevertheless be described as an empirical diagnostic of the produced graph, not as a formally guaranteed validation step of the pipeline itself.

### **2.4 Ensuring Coherence: Entity Resolution with Semantic and Structural Embeddings**

The integrity of the graph's structure, particularly the accurate identification of hubs, depends on a crucial data processing step: Entity Resolution (ER). Raw extraction output can be inconsistent, creating multiple nodes for the same real-world entity (e.g., "Olaf Scholz," "the German Chancellor," "Mr. Scholz"). ER is the process of identifying and merging these duplicates so that each unique entity is represented by a single node in the graph.

In the implemented pipeline, ER is not based on relation-aware knowledge graph embedding models such as TransE. Instead, it is handled through a two-stage normalization strategy. First, lightweight string-based normalization is applied during extraction to collapse obvious surface-form variants within local batches. Second, graph-level semantic resolution compares entity representations in embedding space and merges highly similar nodes. In the iterative experimental workflow, these semantic embeddings may be complemented by optional Node2Vec structural embeddings, allowing both textual similarity and graph position to inform the merge decision.

This process is vital for the validity of the overall analysis. Without effective ER, a central concept like 'Ukraine' might be fragmented into many low-degree nodes, obscuring its role as a hub. By consolidating these fragments into a single, higher-degree node, ER improves the coherence of the entity graph and supports more stable downstream community detection.

### **2.5 Filtering and Pruning as Practical Graph Cleaning**

An earlier version of this project explored more ambitious filtering strategies based on grammatical and semantic heuristics. However, the implemented package pipeline relies primarily on practical graph cleaning steps rather than on a full WordNet-driven abstract-concept filter. In practice, the most important operations before topic modeling are entity normalization, duplicate reduction, and later graph pruning.

This more modest description is methodologically important. The pipeline does not currently depend on an exhaustive linguistic filtering layer in order to produce communities. Instead, it constructs a heterogeneous graph, resolves near-duplicate entities, and then prunes lower-value structure after community detection and summarization. The resulting workflow is therefore better characterized as a cleaned and normalized relational pipeline than as a heavily hand-filtered concept-extraction system.

### **2.6 Topic Modeling as a Use-Case: Structural versus Distributional Approaches**

The application of community detection to the cleaned knowledge graph represents a structural alternative to traditional distributional topic modeling. Whereas LDA and its variants define topics as probability distributions over words—inferring latent structures from co-occurrence patterns—the structural paradigm defined in this thesis treats topics as explicit subgraphs identified through community detection. This distinction has important implications for interpretability, validation, and the philosophical foundations of what constitutes a "topic."
...
**Validation Through Structural Properties**: The structural approach enables diagnostic checks through network properties that are unavailable to distributional methods. In this study, post hoc degree-distribution analysis of saved graph artifacts was used as one such diagnostic. The observed heavy-tailed structure is consistent with the claim that the extracted graph preserves salient discourse hubs rather than producing a purely artificial clustering.
...
The graph cleaning mechanisms described above play an important role in this use-case. By reducing duplication and pruning lower-value structure, they reduce the risk that community detection will identify spurious structures formed by noise entities. The resulting topics are more likely to correspond to genuine thematic divisions in the discourse rather than artifacts of the extraction process.
...
#### **3.1.1 Experimental Enhancement with Node2Vec Embeddings**

While the Leiden algorithm is effective on unweighted graphs, the iterative experimental branch of this project evaluates whether the *strength* of a relation should matter as much as its mere existence. To test this, the workflow optionally applies **Node2Vec** (Grover & Leskovec, 2016) as a structural enhancement before community detection rather than as an unconditional stage of the standard pipeline.

The experimental procedure involves three steps:
1.  **Structural Embedding:** Node2Vec is trained on the current graph to generate low-dimensional vector embeddings for nodes. These embeddings capture topological neighborhood structure and structural role similarity.
2.  **Edge Reweighting:** For existing edges, the cosine similarity between the embeddings of connected nodes is used as an edge weight.
3.  **Weighted Detection:** Leiden is then run on this weighted graph and compared against an unweighted baseline.

Experimental results from this study suggest that this optional weighting can improve community quality. As the size of the graph grew over iterative experiments, the **modularity score** showed a marked improvement when using Node2Vec weights compared to an unweighted baseline. In later iterations (e.g., Iteration 9), modularity improved from **0.748** to **0.823** (+0.075). This quantitative lift indicates that the topological information captured by Node2Vec can help the algorithm carve out sharper thematic boundaries in the experimental setting.

### **3.2 Hierarchical Abstraction of Discourse**

Human discourse is often organized hierarchically. A broad theme, such as "EU's Main Challenges," can be broken down into more specific sub-themes like "The Energy Crisis," "Inflation," and "Disinformation." The implemented pipeline accommodates this structure through a two-level community hierarchy.

After an initial Leiden partition is found, each resulting community is treated as a smaller subgraph and Leiden is run once more within that parent community to identify finer-grained sub-communities. The current implementation therefore produces topics and subtopics, rather than an arbitrarily deep recursive hierarchy. Even this two-level structure is analytically useful, since it allows the analyst to move between broad thematic clusters and more specific line-of-argument groupings within them.
...
This format enforces analytical rigor, ensuring that summaries are not merely descriptive but provide interpretable, evidence-grounded insights. The structured findings are designed to remain close to the supplied graph structure and textual evidence, enhancing transparency, even though the current implementation does not yet perform automatic claim-level verification.
...
If the model were failing (i.e., everything is just "Europe"), we would expect the minimum similarity to be much higher (e.g., >0.50). The presence of these low-similarity pairs confirms that the weighted iterative community-detection experiments can still "cut" the graph at meaningful joints of the discourse, separating technical policy discussions from high-level political rhetoric.
