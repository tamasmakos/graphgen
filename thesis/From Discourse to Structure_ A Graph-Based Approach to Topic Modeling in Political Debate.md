...
**Empirical Observations: LDA on the Same Corpus**: To provide preliminary insight, LDA was applied to the identical corpus of 13 "This is Europe" debate transcripts using scikit-learn's LatentDirichletAllocation with comparable preprocessing (English stop words removed, bigrams included, max features=2000). The results reveal that LDA assigns near-deterministic topic assignments: the mean maximum topic probability per document is 0.92, indicating that each debate is assigned almost entirely to a single topic (probability 0.99-1.00). This is because each debate IS about a specific country—Finland, Greece, Lithuania—and LDA correctly identifies this pre-existing document-level structure. The extracted topics are dominated by country names (Topic 0: Finland/NATO; Topic 7: Romania/Bulgaria/Schengen), directly corresponding to the debate's subject country.

**Important caveats**: This comparison is not methodologically equivalent. LDA operates on bag-of-words distributional representations, while the graph-based approach uses relational entity-relationship structures. Furthermore, LDA with k≈n topics (13 documents, 13 topics) operates outside the typical regime for which LDA was designed—standard evaluation uses coherence metrics (C_v, UMass), not clustering metrics like silhouette scores. A rigorous comparison would require computing topic coherence scores and evaluating on a larger corpus. The observations above should therefore be interpreted as preliminary indicative findings, not definitive comparative conclusions.
