...
**Empirical Comparison: LDA on the Same Corpus**: To provide a fair, direct comparison, LDA was applied to the identical corpus of 13 "This is Europe" debate transcripts using scikit-learn's LatentDirichletAllocation with equivalent preprocessing (English stop words removed, bigrams included, max features=2000). The results reveal several important findings. First, LDA assigns near-deterministic topic assignments: the mean maximum topic probability per document is 0.92, indicating that each debate is assigned almost entirely to a single topic (probability 0.99-1.00). This is because each debate IS about a specific country—Finland, Greece, Lithuania, Croatia—and LDA correctly identifies this document-level structure. Second, LDA topics are dominated by country names: the extracted topics (Topic 0: Finland/NATO; Topic 1: Greece/Turkey; Topic 3: Lithuania/Baltic; Topic 7: Romania/Bulgaria/Schengen) directly correspond to the debate's subject country. Third, silhouette scores are negative: LDA produces silhouette scores ranging from -0.145 (5 topics) to 0.025 (20 topics), indicating poor cluster separation—a finding consistent with the overlapping, multi-perspectival nature of deliberative discourse.

This empirical comparison demonstrates a key difference: LDA discovers the *document structure* (each debate = one topic), while the graph-based approach discovers *relational structure* within debates (entities and their connections across arguments). The graph-based approach can capture sub-topic divisions within a single debate—something LDA's document-level topic model cannot do without substantial modification. This supports the thesis that structural approaches offer a complementary perspective, particularly when the analytical goal is to understand *how topics are discussed* rather than merely *what topics exist*.
...
The research makes three primary contributions to the field of computational discourse analysis.

**Methodologically**, this thesis demonstrates that graph-based topic modeling is a viable and interpretable alternative to probabilistic approaches. The hierarchical summarization pipeline developed here provides a template for translating complex graph structures into human-readable analytical reports, with auditability built into every step. Unlike LDA and its variants, which produce probability distributions whose interpretability requires additional post-processing, the graph-based approach generates explicit knowledge structures where each identified topic is a concrete subgraph that can be examined, validated, and traced back to source documents.

**Theoretically**, the research reveals that standard clustering evaluation metrics—silhouette scores, separation measures—may be inappropriate for deliberative discourse. In domains where consensus-building and shared framing are central features, "overlap" between topics is not a defect but a substantive property. The graph-based approach's ability to distinguish between *relational perspectives* and *semantic topics* offers a new lens for analyzing discourse: where distributional models would see homogeneity, the structural approach can reveal variance in how entities are connected and positioned within argumentative networks.

**Practically**, the approach is particularly well-suited for in-depth analysis of complex, interconnected discourse in domains where interpretability and verifiability are paramount. Political science, policy analysis, and legal text analysis are examples where the ability to trace every claim back to specific entities and relationships may be more valuable than the computational efficiency of probabilistic methods.
